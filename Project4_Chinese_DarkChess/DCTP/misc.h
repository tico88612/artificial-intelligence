...
struct Node {
    ...
    int NonFlipList(int* Board,int color,int *Result) {
        int ResultCount = 0;
        for (int i = 0; i < 32; i++) {
    ...
    int BestFlip() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            if (Board[i] == CHESS_COVER) {
                if (i % 4 != 0 && Board[i - 1] >= 0) {  // 左邊
                    now += flip_neighbor[(Board[i - 1] / 7) ^ Color][Board[i - 1] % 7];
                }
                if (i % 4 != 3 && Board[i + 1] >= 0) {  // 右邊
                    now += flip_neighbor[(Board[i + 1] / 7) ^ Color][Board[i + 1] % 7];
                }
                if (i - 4 >= 0 && Board[i - 4] >= 0) {  // 上面
                    now += flip_neighbor[(Board[i - 4] / 7) ^ Color][Board[i - 4] % 7];
                }
                if (i + 4 < 32 && Board[i + 4] >= 0) {  // 下面
                    now += flip_neighbor[(Board[i + 4] / 7) ^ Color][Board[i + 4] % 7];
                }
                int ii = i - 1, count = 0;
                while (ii >= i / 4 * 4 && count < 2) {  // 左方跳一格
                    if (Board[ii] >= -1) ++count;
                    --ii;
                }
                ...
                ii = i + 1;
                count = 0;
                while (ii < i / 4 * 4 + 4 && count < 2) {  // 右方跳一格
                    if (Board[ii] >= -1) ++count;
                    ++ii;
                }
                if (count == 2 && Board[ii - 1] >= 0) {
...
double Eval(const Node &B) {
    double t = 0;
    int ind = 0;
    int myChess[16];  // 紀錄我方位置
    for (int p = 0; p < 32; p++) {
        int c = (B.Board[p] >= 0) ? B.Board[p] / 7 : -1;  // 算出紅黑方棋子
        if (c == B.Color) {  // 自己的棋子
            t += rate[B.Board[p] % 7] * 10000;
            myChess[ind] = p;
            ++ind;
        } else if (c != -1) {  // 不等於空，也就是敵方棋子
            t -= rate[B.Board[p] % 7] * 10000;
            for (int j = 0; j < ind; j++) {
                int dis = abs(myChess[j] / 4 - p / 4) + abs(myChess[j] % 4 - p % 4);  // 算出與我方棋子的距離
...
double alpha_beta_pruning(Node &B, int dep, int cut, double alpha, double beta, int &BestMove) {
    if (cut == 0) {
        return Eval(B);
    }
    int result[100];
    ...
    if (best_flip == -1 && non_flips == 0) {
        return -WIN + dep;
    }
    if (dep == 0 && best_flip != -1 && non_flips == 0) {  // 只能翻牌
        BestMove = best_flip;
    ...
    for (int i = 0; i < non_flips; i++) {
        ...
        N.MakeMove(move);
        int _;
        double tmp = -alpha_beta_pruning(N, dep + 1, cut - 1, -upb, -std::max(alpha, ret), _);
        if (tmp > ret) {
            if (upb == beta || cut < 3 || tmp >= beta) {
                ...
                ret = alpha_beta_pruning(N, dep + 1, cut - 1, -beta, -tmp, _);
                BestMove = result[i];
                if (dep == 0) {
                    fprintf(stderr, "%s score= %.2lf\n", move, tmp);
                }
    ...
    if (best_flip != -1) {
        ...
        for (int i = 0; i < 14; i++) {
            if (B.CloseChess[i] > 0) {
                ...
                N.MakeMove(move);
                int _;
                double tmp = -alpha_beta_pruning(N, dep + 1, cut - 1, -INF, INF, _);
